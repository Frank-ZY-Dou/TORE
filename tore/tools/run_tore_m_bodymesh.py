...
import numpy as np
import cv2
from tore.modeling_m.bert import BertConfig, METRO
from tore.modeling_m.bert import METRO_Body_Network as METRO_Network
from tore.modeling_m._smpl import SMPL, Mesh
...
from tore.utils.renderer_metro import Renderer, visualize_reconstruction, visualize_reconstruction_test
    ...
    for iteration, (img_keys, images, annotations) in tqdm(enumerate(train_dataloader)):
        ...
        loss = args.joints_loss_weight*loss_3d_joints + \
                args.vertices_loss_weight*loss_vertices  + args.vertices_loss_weight*loss_2d_joints \
                     + 0.1 * loss_normal_vector + 1.0 * loss_edge_length
...
def parse_args():
    ...
    parser.add_argument("--model_name_3", default='FastMETRO_L', type=str)
    parser.add_argument("--feedforward_dim_3", default=512, type=int)
    parser.add_argument("--transformer_dropout_3", default=0.1, type=float)
    parser.add_argument("--transformer_nhead_3", default=8, type=int)
    parser.add_argument("--pos_type_3", default='sine', type=str)
...
def main(args):
    ...
    if args.run_eval_only==True and args.resume_checkpoint!=None and args.resume_checkpoint!='None' and 'state_dict' not in args.resume_checkpoint:
        ...
        if args.arch=='hrnet':
            ...
            resnet50_ckpt = "./models/resnet/resnet50-0676ba61.pth"
            # state_dict = torch.load(resnet50_ckpt, map_location='cuda:{}'.format(args.local_rank))
            state_dict = torch.load(resnet50_ckpt, map_location=args.device)
            backbone = resnet50()
