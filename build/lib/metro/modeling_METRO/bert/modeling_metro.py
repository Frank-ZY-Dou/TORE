...
class METRO_Body_Network(torch.nn.Module):
    ...
    def __init__(self, args, config, backbone, trans_encoder, mesh_sampler):
        ...
        self.conv_learn_tokens = torch.nn.Conv1d(49,14,1)

        self.num_joints = 14
        if 'FastMETRO_L' in args.model_name_3:
            num_enc_layers_3 = 3
            num_dec_layers_3 = 3
        last_hidden = int(args.hidden_feat_dim.split(",")[-1])
        self.transformer_config_3 = {"model_dim": last_hidden, "dropout": args.transformer_dropout_3, "nhead": args.transformer_nhead_3,
                                     "feedforward_dim": args.feedforward_dim_3, "num_enc_layers": num_enc_layers_3, "num_dec_layers": num_dec_layers_3,
                                     "pos_type": args.pos_type_3}

        self.cam_token_embed = nn.Embedding(1, int(self.transformer_config_3["model_dim"]))
        self.vertex_token_embed = nn.Embedding((431), int(self.transformer_config_3["model_dim"]))
        self.transformer_3 = build_transformer(self.transformer_config_3)
        self.position_encoding_3 = build_position_encoding(pos_type=self.transformer_config_3['pos_type'], hidden_dim=last_hidden)
        self.cam_predictor = nn.Linear(last_hidden, 3)
        self.xyz_regressor_v = nn.Linear(last_hidden, 3)
        ...
        self.attention_mask_3 = temp_mask_2
    ...
    def forward(self, images, smpl, mesh_sampler, meta_masks=None, is_train=False):
        ...
        # concatenate template joints and template vertices, and then duplicate to batch size
        ref_vertices = template_3d_joints
        ref_vertices = ref_vertices.expand(batch_size, -1, -1)
        ...
        # concatenate image feat and template mesh
        ...
        if self.config.output_attentions==True:
            features, hidden_states, att = self.trans_encoder(features)
            joint_feature_2 =  hidden_states[-1]
        ...
        v_attention_mask = self.attention_mask_3.to(images.device)
        pos_enc_3 = self.position_encoding_3(batch_size, self.num_joints, 1, images.device).flatten(2).permute(2, 0, 1)
        joint_feature_2 = joint_feature_2.permute(1,0,2)

        # GTR
        cam_features_3, _, v_features = self.transformer_3(joint_feature_2, cam_token, v_tokens, pos_enc_3,
                                              attention_mask=v_attention_mask)
        pred_3d_joints = features


        pred_3d_coordinates_V = self.xyz_regressor_v(v_features.transpose(0, 1))

        pred_vertices_sub2 = pred_3d_coordinates_V.transpose(1,2)

        # learn camera parameters
        cam_param = self.cam_predictor(cam_features_3).view(batch_size, 3)

        pred_vertices_sub = self.upsampling(pred_vertices_sub2)
        ...
        pred_vertices_sub2 = pred_vertices_sub2.transpose(1,2)
