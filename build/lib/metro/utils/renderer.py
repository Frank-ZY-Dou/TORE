...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    elif joints.shape[1] not in [431, 1723, 6890]:
        print('Unknown skeleton!!')

    if joints.shape[1] in [431, 1723, 6890]:
...
def visualize_reconstruction(img, img_size, gt_kp, vertices, pred_kp, camera, renderer, mesh_2d, color='pink', focal_length=1000):
    ...
    mesh_2d = ((mesh_2d[:, :2] + 1) * 0.5) * img_size
    ...
    mesh_2d_img = draw_skeleton(img, mesh_2d, draw_edges=False)

    combined = np.hstack([skel_img, rend_img, mesh_2d_img])

    return combined

def visualize_reconstruction_inference(img, vertices, camera, renderer, color='light_blue', focal_length=1000):
    """Overlays gt_kp and pred_kp on img.
    Draws vert with text.
    Renderer is an instance of SMPLRenderer.
    """
